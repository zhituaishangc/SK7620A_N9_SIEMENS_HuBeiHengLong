...
|||
|TOOL_SET[5]|对刀开始时的C角度用于计算磨削起点对应的角度
|INI[49]|自动对刀开始前的C角度,为了避开转向器孔。与上一项的值相等。
|R279|Z向补偿砂轮偏刀时通过该数值调整,通过动起点实现
|R278|自动对刀值不准确时通过该数值调整(通过改变起点角度来实现)
|R277磨削长度
|R276|刀补上限
|R275|单次刀补
|R274|刀补下限
...
|TOOL_SET[40]||手动输入的C起始角
|TOOL_SET[44]|测头在工件内感应到信号时的X坐标|
|||
|**滚轮参数(WHEEL)**||
...
|WHEEL[20]|砂轮修到最小时的X接触坐标|
|WHEEL[21]|新砂轮修整到位后的X接触坐标|
|||
|**三角齿形(TRI)**||
